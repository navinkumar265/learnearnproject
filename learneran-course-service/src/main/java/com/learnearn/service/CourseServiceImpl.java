...
public class CourseServiceImpl implements ICourseService {
	...
	@Override
	public Course addCourse(Course course,int companyId,int batchId) {
		...
		course.setCompany(companyNew);
		String  batchUrl="http://localhost:8082/batch-service/batches/"+batchId;
		ResponseEntity<Batch> batch=restTemplate.getForEntity(batchUrl,Batch.class);
		Batch batchNew =batch.getBody();
		course.setBatch(batchNew);
	...
	public List<Trainer> viewTrainer() {
		String url=BASEURL+"/view-trainers";
		ResponseEntity<List>  trainerList=restTemplate.getForEntity(url, List.class);
		return trainerList.getBody();
	}
	...
	public void assignTrainer(int courseId, int trainerId) {
		String url=BASEURL+"/assigntrainer/courseId/"+courseId+"/trainerId/"+trainerId;
		restTemplate.getForEntity(url, Trainer.class);
		Course course = courseRepository.findById(courseId).get();
		course.setStatus(PostStatus.DEFINED);
		courseRepository.save(course);
	}
	...
	public void deAssignTrainer(int courseId,int trainerId) {
		String url=BASEURL+"/deassigntrainer/trainerId/"+trainerId;
		restTemplate.getForEntity(url, Trainer.class);
		Course course = courseRepository.findById(courseId).get();
		course.setStatus(PostStatus.COMPLETED);
		courseRepository.save(course);
